...
def get_data(company_code, company_name):
	...
	# 데이터 날짜
	date = bs_obj.find('em',{'class':'date'}).text
	...
	# percent
	print(bs_obj.find('div',{'class':'rate_info'}).find('p',{'class':'no_exday'}).findAll('em')[1])
	percent = bs_obj.find('div',{'class':'rate_info'}).find('p',{'class':'no_exday'}).findAll('em')[1].find('span',{'class','blind'})
	sign = bs_obj.find('div',{'class':'rate_info'}).find('p',{'class':'no_exday'}).findAll('em')[1].findAll('span')[0]
	...
	# 거래 대금
	transaction_price = bs_obj.find('table',{'class':'no_info'}).findAll('tr')[1].findAll('td')[2].find('span',{'class','blind'})
	if not transaction_price : return

	# 거래량
	Volume = bs_obj.find('table',{'class':'no_info'}).findAll('tr')[0].findAll('td')[2].find('span',{'class','blind'})
	...
	# print(get_text(transaction_price))

	if int(get_text(transaction_price)) > 10000 and chk_number(get_text(A_ROE)) and chk_number(get_text(Q_ROE)) and chk_number(get_text(A_PER)) and chk_number(get_text(Q_PER)):
		...
		if f_A_ROE / f_A_PER > 0 and f_Q_ROE / f_Q_PER > 0:
			return {
				"company_name" : company_name,
				"company_code":company_code ,
				"percent":get_text(percent) ,
				"cur_price":blind_now.text,
				"transaction_price":get_text(transaction_price),
...
file = open('./trading_200830_02.json','w+')
